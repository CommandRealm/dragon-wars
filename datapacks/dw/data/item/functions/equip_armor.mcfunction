execute unless entity @s[tag=!iron,tag=!chain,tag=!ninja,tag=!lucky,tag=!spartan,tag=!gladiator,tag=!jump_boost] run scoreboard players set @s armor_delay 100
...
execute if entity @e[type=item,distance=..10,nbt={Item:{tag:{ArmorType:6}},Age:1s}] run function item:equip_jump_boost