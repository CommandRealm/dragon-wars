...
# death scroll
execute as @a[scores={use_d_scroll=1..}] at @s run function item:drop_death_scroll
...
# if a sniping arrow exists
execute if entity @e[type=arrow,tag=sniping_arrow] run function item:active_sniping_arrow

# when a player successfully blocks with a shield
execute as @a[scores={use_shield=1..}] at @s run function item:use_shield

# fake strength
execute if entity @a[scores={fake_strength=1..}] run function item:active_fake_strength


scoreboard players remove @a[scores={firework_shield_cool=1..}] firework_shield_cool 1