


##Removing tags
tag @a remove has_test
tag @a remove has_help
tag @a remove has_bld
tag @a remove has_prg
tag @a remove has_srbld
tag @a remove has_srprg
tag @a remove has_pc
tag @a remove has_manager
tag @a remove has_lead
tag @a remove has_art
tag @a remove has_cr
tag @a remove has_tournament



tag Bodi25 add has_pc
tag pcgus add has_pc


##Those who have tester
tag FalconReign add has_test
tag Connop add has_test
tag TheRangerMark add has_test
tag SmallDeemo add has_test
tag Captonium add has_test
tag CheckMC add has_test
tag ChipChase add has_test
tag Bodi25 add has_test
tag Nxmes remove has_test
tag Nxmes add has_test
tag TheRegularMex add has_test
tag Arrgs add has_test
tag TheTypicalMex add has_test
tag PurpleSimp add has_test
tag tinaaaa420 add has_test
tag II_Owen_II add has_test
tag GeneralGuy64 add has_test
tag VoidGolem add has_test
tag Calverin add has_test
tag Focii_ add has_test
tag physa add has_test
tag GoldSlimeling91 add has_test
tag J70 add has_test
tag CoolCheeto2 add has_test
tag Leesobee add has_test
tag endermint06 add has_test
tag T_Man_Happy add has_test
tag Theluco617 add has_test
tag Peeps__ add has_test
tag ZachEP add has_test
tag berglerr add has_test
tag NauticaElements add has_test
tag BelenDrane add has_test
tag Warrior0120 add has_test
tag gaysaregreat add has_test
tag OPWolf2323 add has_test
tag EpicStar123 add has_test
tag Business_Pug add has_test
tag Cosmic_Evil add has_test
tag CrazyPig19 add has_test
tag matthoff8 add has_test
tag Pulluxx add has_test
tag Moesh add has_test
tag pcgus add has_test
tag FlameAndDiamond add has_test
tag Bagee add has_test
tag BobaFate add has_test
tag EdLePug add has_test
tag oPando add has_test

tag 3letters_ add has_test
tag ces42 add has_test
tag Dashproworld12 add has_test
tag GameGenMC add has_test
tag JaxF2008 add has_test
tag _Lexist_ add has_test
tag Shullker add has_test
tag Nerviezz add has_test
tag SilentKing_ add has_test
tag SinfulBlood1366 add has_test
tag 4953makcim add has_test
tag Ardak78 add has_test
tag sadkina2 add has_test
tag TheViralMelon add has_test


# help
tag FalconReign add has_help
tag Connop add has_help
tag TheRangerMark add has_help
tag SmallDeemo add has_help
tag Captonium add has_help
tag CheckMC add has_help
tag ChipChase add has_help
tag Bodi25 add has_help
tag Nxmes remove has_help
tag TheRegularMex add has_help
tag TheTypicalMex add has_help
tag tinaaaa420 add has_help
tag II_Owen_II add has_help
tag Calverin add has_help
tag Focii_ add has_help
tag J70 add has_help
tag EdLePug add has_help
tag oPando add has_help
tag Icy_Craby add has_help
tag joelzoboyo add has_help
tag Coraze add has_help

# bld
tag FalconReign add has_bld
tag Connop add has_bld
tag TheRangerMark add has_bld
tag SmallDeemo add has_bld
tag Captonium add has_bld
tag CheckMC add has_bld
tag ChipChase add has_bld
tag Bodi25 add has_bld
tag Nxmes remove has_bld
tag TheRegularMex add has_bld
tag tinaaaa420 add has_bld
tag J70 add has_bld

# bld
tag Connop add has_srbld

# prg
tag FalconReign add has_prg

# srprg
tag FalconReign add has_srprg

# manager
tag FalconReign add has_manager
tag Connop add has_manager
tag Captonium add has_manager


tag FalconReign add has_cr
tag Connop add has_cr
tag TheRegularMex add has_cr
tag Captonium add has_cr
tag TheRangerMark add has_cr
tag TheTypicalMex add has_cr
tag Calverin add has_cr
tag ChipChase add has_cr
tag CheckMC add has_cr
tag SmallDeemo add has_cr
tag Focii_ add has_cr
tag pcgus add has_cr
tag Bodi25 add has_cr
tag EdLePug add has_cr
tag Bagee add has_cr
tag BobaFate add has_cr
tag knad add has_cr
tag Urgle add has_cr
tag NotInteligent add has_cr
tag Coraze add has_cr
tag rgbyy add has_cr
tag bluepower add has_cr
tag Beybwug add has_cr
tag Mezimo add has_cr
tag Shullker add has_cr
tag Pinkessi add has_cr


# organizers
tag FalconReign add has_organizer
tag Connop add has_organizer
tag EdLePug add has_organizer

# has_tournament
tag Shullker add has_tournament
tag 3letters_ add has_tournament
tag J70 add has_tournament
tag 4953makcim add has_tournament
tag Bretos add has_tournament
tag 1ona add has_tournament


# tournament sword names

tag @a remove has_t_j22_3
tag @a remove has_t_j22_2
tag @a remove has_t_j22_1

tag Shullker add has_t_j22_1
tag 3letters_ add has_t_j22_1
tag J70 add has_t_j22_2
tag 4953makcim add has_t_j22_3
tag Bretos add has_t_j22_3
tag 1ona add has_t_j22_2