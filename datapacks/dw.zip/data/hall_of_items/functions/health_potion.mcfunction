setblock 69 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Health Potion"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Health Potion"]
...
execute if entity @a[tag=toggle] run setblock 69 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"Healing Potion","color":"light_purple","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:health_potion"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 69 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"Healing Potion","color":"light_purple","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:health_potion"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 69 73 -2 {Item:{id:"minecraft:splash_potion",Count:1b,tag:{Potion:"healing"}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_health_potion"]}
