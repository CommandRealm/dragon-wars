setblock 70 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Speed Potion"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Speed Potion"]
execute if entity @a[tag=toggle] run summon minecraft:armor_stand -2 56 26 {CustomName:'"Speed Potion"',Tags:["item_spawn"],Invulnerable:1}
playsound minecraft:ui.button.click master @s[tag=!no_msg] ~ ~ ~ 100000 1
execute if entity @a[tag=toggle] run setblock 70 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Hyperspeed","color":"aqua","bold":false}', '{"text":"Potion","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:hyperspeed_potion"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 70 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Hyperspeed","color":"aqua","bold":false}', '{"text":"Potion","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:hyperspeed_potion"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 70 73 -2 {Item:{id:"minecraft:potion",Count:1b,tag:{Potion:"swiftness"}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_speed_potion"]}
execute unless entity @a[tag=toggle] run kill @e[tag=model_speed_potion]
tag @a remove toggle
