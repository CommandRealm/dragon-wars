setblock 59 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Multishot"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Multishot"]
...
execute if entity @a[tag=toggle] run setblock 59 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Multishot","color":"dark_red","bold":false}', '{"text":"Crossbow","color":"dark_red","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:multishot_crossbow"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 59 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Multishot","color":"dark_red","bold":false}', '{"text":"Crossbow","color":"dark_red","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:multishot_crossbow"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 59 73 -2 {Item:{id:"minecraft:crossbow",Count:1b,tag:{Enchantments:[{id:"easter_egg",lvl:1}]}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_multishot"]}
