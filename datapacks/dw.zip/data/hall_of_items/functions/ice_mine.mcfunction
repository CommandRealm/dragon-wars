setblock 73 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Ice Mine"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Ice Mine"]
...
execute if entity @a[tag=toggle] run setblock 73 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Ice","color":"aqua","bold":false}', '{"text":"Mine","color":"aqua","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:ice_mine"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 73 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Ice","color":"aqua","bold":false}', '{"text":"Mine","color":"aqua","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:ice_mine"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 73 73 -14 {Item:{id:"minecraft:ice",Count:3b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_ice_mine","no_activate"]}
