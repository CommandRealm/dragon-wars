setblock 70 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Strength Shield"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Strength Shield"]
execute if entity @a[tag=toggle] run summon minecraft:armor_stand -3 56 26 {CustomName:'"Strength Shield"',Tags:["item_spawn"],Invulnerable:1}
playsound minecraft:ui.button.click master @s[tag=!no_msg] ~ ~ ~ 100000 1
execute if entity @a[tag=toggle] run setblock 70 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Strength","color":"dark_red","bold":false}', '{"text":"Shield","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:strength_shield"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 70 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Strength","color":"dark_red","bold":false}', '{"text":"Shield","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:strength_shield"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 70 73 -14 {Item:{id:"minecraft:shield",Count:1b,tag:{BlockEntityTag:{Base:15,Patterns:[{Color:7,Pattern:"mr"},{Color:8,Pattern:"flo"},{Color:14,Pattern:"mc"},{Color:15,Pattern:"cr"}],id:"minecraft:banner"},Damage:0}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_strength_shield"]}
execute unless entity @a[tag=toggle] run kill @e[tag=model_strength_shield]
tag @a remove toggle
