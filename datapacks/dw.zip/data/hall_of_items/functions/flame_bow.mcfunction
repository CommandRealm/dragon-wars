setblock 55 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Flame Bow"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Flame Bow"]
...
execute if entity @a[tag=toggle] run setblock 55 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"Fire Bow","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:flame_bow"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 55 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"Fire Bow","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:flame_bow"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 55 73 -2 {Item:{id:"minecraft:bow",Count:1b,tag:{Enchantments:[{id:"easter_egg",lvl:1}]}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_flame"]}
