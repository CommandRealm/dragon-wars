setblock 74 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Reflector"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Reflector"]
execute if entity @a[tag=toggle] run summon minecraft:armor_stand -3 56 25 {CustomName:'"Reflector"',Tags:["item_spawn"],Invulnerable:1}
playsound minecraft:ui.button.click master @s[tag=!no_msg] ~ ~ ~ 100000 1
execute if entity @a[tag=toggle] run setblock 74 72 -13 jungle_wall_sign[facing=south]{Text1:'',Text2:'{"text":"Reflector","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:reflector"}}',Text3:'{"text":"[ON]","color":"green"}',Text4:''}
execute unless entity @a[tag=toggle] run setblock 74 72 -13 jungle_wall_sign[facing=south]{Text1:'',Text2:'{"text":"Reflector","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:reflector"}}',Text3:'{"text":"[OFF]","color":"dark_red"}',Text4:''}
