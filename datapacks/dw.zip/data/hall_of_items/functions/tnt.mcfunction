setblock 64 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="TNT"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="TNT"]
...
execute if entity @a[tag=toggle] run setblock 64 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"TNT","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:tnt"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 64 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":""}', '{"text":"TNT","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:tnt"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 64 73 -2 {Item:{id:"minecraft:tnt",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_tnt"]}
