setblock 62 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Thief Beam"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Thief Beam"]
execute if entity @a[tag=toggle] run summon minecraft:armor_stand -3 56 26 {CustomName:'"Thief Beam"',Tags:["item_spawn"],Invulnerable:1}
playsound minecraft:ui.button.click master @s[tag=!no_msg] ~ ~ ~ 100000 1
execute if entity @a[tag=toggle] run setblock 62 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Thief","color":"yellow","bold":false}', '{"text":"Beam","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:thief_beam"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 62 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Thief","color":"yellow","bold":false}', '{"text":"Beam","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:thief_beam"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 62 73 -2 {Item:{id:"minecraft:magma_cream",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_thief_beam"]}
execute unless entity @a[tag=toggle] run kill @e[tag=model_thief_beam]
tag @a remove toggle
