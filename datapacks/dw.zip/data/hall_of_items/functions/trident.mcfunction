setblock 72 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Trident"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Trident"]
...
execute if entity @a[tag=toggle] run setblock 72 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Trident","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:trident"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 72 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Trident","color":"dark_aqua","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:trident"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 72 73 -14 {Item:{id:"minecraft:trident",Count:1b,tag:{Enchantments:[{id:"easter_egg",lvl:1}]}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_trident"]}
