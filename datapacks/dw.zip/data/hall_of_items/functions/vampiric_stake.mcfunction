setblock 57 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Vampiric Stake"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Vampiric Stake"]
...
execute if entity @a[tag=toggle] run setblock 57 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Vampiric Stake","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:vampiric_stake"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 57 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Vampiric Stake","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:vampiric_stake"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 57 73 -14 {Item:{id:"minecraft:wooden_sword",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_stake"]}
