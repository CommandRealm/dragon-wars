setblock 75 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Death Scythe"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Death Scythe"]
...
execute if entity @a[tag=toggle] run setblock 75 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Death\'s","color":"dark_gray","bold":false}', '{"text":"Scroll","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:death_scythe"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 75 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Death\'s","color":"dark_gray","bold":false}', '{"text":"Scroll","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:death_scythe"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 75 73 -14 {Item:{id:"minecraft:map",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_death_scythe","unusable"]}
