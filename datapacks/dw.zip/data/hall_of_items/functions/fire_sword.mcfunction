setblock 55 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Fire Sword"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Fire Sword"]
...
execute if entity @a[tag=toggle] run setblock 55 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Fire Sword","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:fire_sword"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 55 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Fire Sword","color":"red","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:fire_sword"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 55 73 -14 {Item:{id:"minecraft:golden_sword",tag:{Enchantments:[{id:"fire_aspect",lvl:1}]},Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_f_sword"]}
