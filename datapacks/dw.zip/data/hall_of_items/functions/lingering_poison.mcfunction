setblock 71 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Lingering Poison"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Lingering Poison"]
...
execute if entity @a[tag=toggle] run setblock 71 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Lingering Potion","color":"green","bold":false}', '{"text":"of Poison","color":"green","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:lingering_poison"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 71 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Lingering Potion","color":"green","bold":false}', '{"text":"of Poison","color":"green","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:lingering_poison"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 71 73 -2 {Item:{id:"minecraft:lingering_potion",Count:1b,tag:{Potion:"long_poison"}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_poison_potion"]}
