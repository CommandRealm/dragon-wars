setblock 62 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Chain Armor"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Chain Armor"]
...
execute if entity @a[tag=toggle] run setblock 62 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Archery Prot","color":"dark_gray","bold":false}', '{"text":"Armor","color":"dark_gray","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:archery_armor"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 62 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Archery Prot","color":"dark_gray","bold":false}', '{"text":"Armor","color":"dark_gray","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:archery_armor"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 62 73 -14 {Item:{id:"minecraft:firework_star",Count:1b,tag:{Explosion:{Colors:[I;4934475]}}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_archery"]}
