setblock 65 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Time Bomb"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Time Bomb"]
...
execute if entity @a[tag=toggle] run setblock 65 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Time","color":"dark_gray","bold":false}', '{"text":"Bomb","color":"dark_gray","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:time_bomb"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 65 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Time","color":"dark_gray","bold":false}', '{"text":"Bomb","color":"dark_gray","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:time_bomb"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 65 73 -2 {Item:{id:"minecraft:chiseled_polished_blackstone",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_time_bomb","unusable"]}
