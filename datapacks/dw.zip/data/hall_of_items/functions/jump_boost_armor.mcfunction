setblock 67 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Leaping Armor"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Leaping Armor"]
...
execute if entity @a[tag=toggle] run setblock 67 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Leaping","color":"green","bold":false}', '{"text":"Armor","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:jump_boost_armor"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 67 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Leaping","color":"green","bold":false}', '{"text":"Armor","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:jump_boost_armor"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 67 73 -14 {Item:{id:"minecraft:firework_star",Count:1b,tag:{Explosion:{Colors:[I;4977989]}}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_jump_boost_armor","unusable","jump_boost_armor"]}
