setblock 63 72 -3 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Levitation Blast"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Levitation Blast"]
...
execute if entity @a[tag=toggle] run setblock 63 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Levitation","color":"#f9baff","bold":false}', '{"text":"Blast","color":"#f9baff","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:levitation_blast"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 63 72 -3 cherry_wall_sign[facing=north]{front_text:{messages:['{"text":"Levitation","color":"#f9baff","bold":false}', '{"text":"Blast","color":"#f9baff","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:levitation_blast"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 63 73 -2 {Item:{id:"minecraft:end_crystal",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_levitation_blast","unusable"]}
