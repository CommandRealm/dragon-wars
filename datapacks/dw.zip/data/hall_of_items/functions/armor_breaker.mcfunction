setblock 58 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Armor Breaker"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Armor Breaker"]
...
execute if entity @a[tag=toggle] run setblock 58 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Armor","color":"gray","bold":false}', '{"text":"Breaker","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:armor_breaker"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 58 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":"Armor","color":"gray","bold":false}', '{"text":"Breaker","color":"gold","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:armor_breaker"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 58 73 -14 {Item:{id:"minecraft:iron_pickaxe",Count:1b},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_armor_breaker","unusable"]}
