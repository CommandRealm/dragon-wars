setblock 61 72 -13 air
execute unless entity @a[tag=no_toggle] unless entity @e[type=armor_stand,tag=item_spawn,name="Iron Armor"] run tag @p add toggle
kill @e[type=armor_stand,tag=item_spawn,name="Iron Armor"]
execute if entity @a[tag=toggle] run summon minecraft:armor_stand 0 56 24 {CustomName:'"Iron Armor"',Tags:["item_spawn"],Invulnerable:1}
playsound minecraft:ui.button.click master @s[tag=!no_msg] ~ ~ ~ 100000 1
execute if entity @a[tag=toggle] run setblock 61 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Iron Armor","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/function hall_of_items:iron_armor"}}', '{"text":"[ON]","color":"green"}', '{"text":""}']}}
execute unless entity @a[tag=toggle] run setblock 61 72 -13 cherry_wall_sign[facing=south]{front_text:{messages:['{"text":""}', '{"text":"Iron Armor","color":"gray","bold":false,"clickEvent":{"action":"run_command","value":"/execute if score $game state matches 0 run function hall_of_items:iron_armor"}}', '{"text":"[OFF]","color":"dark_red"}', '{"text":""}']}}
execute if entity @a[tag=toggle] run summon item 61 73 -14 {Item:{id:"minecraft:firework_star",Count:1b,tag:{Explosion:{Colors:[I;16777215]}}},Age:-30000,Invulnerable:1b,PickupDelay:10124151,Tags:["item_model","item_temp","model_iron"]}
execute unless entity @a[tag=toggle] run kill @e[tag=model_iron]
tag @a remove toggle
