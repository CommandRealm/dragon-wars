...
execute unless entity @e[type=armor_stand,tag=parkour_leaderboard,tag=temporary_tag_2] run tag @e[type=armor_stand,tag=parkour_leaderboard,scores={leaderboard_pos=2}] add temporary_tag_2
...
data modify block 0 50 0 front_text.messages[1] set value '[{"text":"","italic":false},{"selector":"@p[tag=temporary_tag]"},{"text":"\'s ","color":"yellow"},{"text":"time: ","color":"gray"},{"nbt":"front_text.messages[0]","block":"0 50 0","interpret":true}]'
data modify entity @e[type=armor_stand,tag=parkour_leaderboard,tag=temporary_tag_2,limit=1] CustomName set from block 0 50 0 front_text.messages[1]
